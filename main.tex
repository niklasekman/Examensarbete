...
\begin{figure}[H]
    ...
    \caption{Profiler för en flygplansvinge (ovan) och ett konventionellt segel samt mast (under)\waller{Varifrån är bilden? Brukar vara noga med referenser för bilder.}}
...
$C_l$ tas fram antingen experimentellt i vindtunnel eller genom simulering vid olika attackvinklar av vingen mot luftströmmen. Koefficienten gäller för ett visst Reynoldstal, $Re$ 
\waller{(strikt taget skall man skriva Reynolds tal som $\mathrm{Re}$, lutande bokstäver $Re$ betecknar matematiskt produkten av $R$ med $e$)}, som kan ses som en skalfaktor \citep{Enqvist2016-xw} som beaktar en karakteristisk dimension som oftast är vingens längd i tvärsnittet $L$, fluidens hastighet $v$ och fluidens dynamiska viskositet $\mu$ \citep{Waller_undated-el}. 
...
Så länge som förhållandet mellan $\sigma$ och $\varepsilon$ är linjärt kan utböjningen $\delta$ bestämmas med lätthet. För samma belastningsfall som för ekv. \ref{eqn:bojmoment} ges $\delta$ av ekv. \ref{eqn:utbojning}. Samma ekvation kan användas för att uppskatta $E$ utifrån experimentell data. Tröghetsmomentet $I_x$ för ett rör \waller{Gäller detta för ett rör som kan rotera kring ena ändan?} ges av ekvation \ref{eqn:troghet}.
...
Med instrumentering av masten kan kraften seglet skapar mätas direkt till skillnad från att förlita sig på skenbar vindriktning \waller{Dessutom finns antagandet om att den skenbara vindhastigheten är konstant över hela höjden på seglet med, knappast riktigt det heller. Kan också nämnas om du vill.} och hastighet samt $C_L$ och $C_D$ tabeller.
...
\begin{figure}[H]
    ...
    \caption{Töjningnsgivarna fastlimmade \waller{Det ser ut som de är monterade i rätt riktning?}}
    \label{fig:mast_2}
\end{figure}
...
\waller{Kanske en egen sektion med underrubrik? Och eventuellt en bild av blockschemat för att illustrera LabVIEW-programmering?}

Ett relativt omfattande labviewprogram förbereddes för kalibrering och mätningar med bryggan.

Programmet har ett gränssnitt som genom flikar är indelat i de tre faser man behöver gå igenom för att först samla data för kalibreringen, sedan utföra en linjär anpassning av samlad data och till sist utföra mätningar och lagra data. Dessa presenteras i figurerna \ref{fig:prog_cal}, \ref{fig:prog_fit} och \ref{fig:prog_meas}. \waller{Kanske man borde förklara lite om gränssnitten och vad användaren behöver göra, dvs val som antal mätningar per riktning, etc.,...}
...
Det egentliga praktiska experimentet som all tidigare beskriven utrustning och teori bygger mot beskrivs nedan. Det består av två olika skeden, en kalibrering och en mätning. Kalibreringen behöver göras eftersom värdet som fås från AD-omvandlaren inte direkt motsvarar någon känd kraft som verkar på masten \waller{Lite förvillande, i förra kapitlet fick jag uppfattningen att du räknar ut ett teoretiskt fall som beaktar geometri och material och vilket utslag man skulle få? Vad gör då egentligen en skild kalibrering?}. Målet med mätningen är att se hur väl kraften som påverkar masten kan mätas då givarna i de båda bryggorna töjs i en riktning som inte motsvarar deras rekommenderade töjningsriktning, enligt diskussionen i avsnitt \ref{subsect:kraftmatning}. \waller{Å andra sidan förstår jag denna mening, är det då så att det finns ett teoretiskt fall i en viss riktning som man i princip räkna utslaget för?}
...
Så länge som materialen befinner sig i den elastiska regionen kommer förhållandet mellan $\sigma$ och $\varepsilon$ vara linjärt \waller{Behöver man beakta att rörets tjocklek i olika riktningar ändras vid böjning, eller är böjningen såpass liten?}. Detta betyder att en linjär anpassning av mätvärden kan göras för att få fram en kalibreringsfaktor. 
