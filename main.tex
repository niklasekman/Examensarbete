...
\section{Servomotorer}\label{sect:servomotorer}
...
En 3D-utskrift som håller fast den diametriskt polariserade magneten limmades fast på det stora kugghjulet som är kopplat till motorns axel (figur \ref{fig:mot_magn}). Ett nytt lock för motorns växelhus som kretskortet med hallgivarchipet kunde fästas vid ritades också upp på basen av en inscannad bild av packningen som satt under det ursprungliga locket och skrevs ut (fig. \ref{fig:mot_magn}). 

\begin{figure}[H]
    \centering
    \includegraphics[scale=.35]{IMG_0337_korr.jpg}
    \caption{Magnet fastlimmad i vindrutetorkarmotorns växelhus}
    \label{fig:mot_magn}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale=.1]{lock.jpeg}
    \caption{Locket som encodern kan fästas vid}
    \label{fig:mot_lock}
\end{figure}

För hastighetsreglering av motorn användes en färdig modul som innehöll en H-brygga, specifikt avsedd för att styra likströmsmotorer. Modulen matades med 12 V spänning från ett labbaggregat som kan leverera en maximal ström på 5 A. Mikrokontrollern programmerades att skicka ut en PWM-signal för det önskade pådraget och en binär signal som anger riktningen till modulen. En prototyp av arrangemanget kan ses i figur \ref{fig:mot_proto}.

\begin{figure}[H]
    \centering
    \includegraphics[scale=.35]{IMG_0367_korr.jpg}
    \caption{Prototyp av servomotor}
    \label{fig:mot_proto}
\end{figure}
...
Mikrokontrollern programmerades med ett arduinoprogram som läser in vinkeln från encodern och rapporterar tillbaka värdet över serieporten till datorn. Programmet tar också emot ett värde mellan -255 och 255 för motorpådraget. Negativa värden får motorn att gå moturs och positiva medurs.

En simulinkmodell som skickar ut  byggdes 
